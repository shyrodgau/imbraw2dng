...
version = "V3.3.0_2087006"; // actually const
...
handleone(orientation) {
	...
	reader.onload = (evt) => {
		...
		out.set(rest2, k);
		k += rest2.length;
		this.writeinttoout(out, f.size + (dateaddoff + 302), k);
		k += 4;

		const rest3 = [ 0x10, 0x01, 0x02, 0x00, 0x08, 0x00, 0x00, 0x00 ];
		out.set(rest3, k);
		k += rest3.length;
		this.writeinttoout(out, f.size + (dateaddoff + 317), k);
		k += 4;
		...
		out.set(rest5b, k);
		k += rest5b.length;
		this.writeinttoout(out, f.size + (dateaddoff + 326), k);
		k += 4;
		...
		if (dateok) { // one datetime tag
			...
			out.set(rest5a, k);
			k += rest5a.length;
			this.writeinttoout(out, f.size + 466 + veroff, k);
			k += 4;
		}
		...
		out.set(rest6b, k);
		k += rest6b.length;
		this.writeinttoout(out, (dateaddoff + 310) + f.size, k);
		k += 4;

		const rest7 = [ 0x1d, 0xc6, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0x21, 0xc6, 0x0a, 0x00, 0x09, 0x00, 0x00, 0x00 ];
		out.set(rest7, k);
		k += rest7.length;
		this.writeinttoout(out, (dateaddoff + 346 + veroff) + f.size, k);
		k += 4;

		const rest8 = [ 0x28, 0xc6, 0x05, 0x00, 0x03, 0x00, 0x00, 0x00 ];
		out.set(rest8, k);
		k += rest8.length;
		this.writeinttoout(out, (dateaddoff + 418 + veroff) + f.size, k);
		k += 4;

		const rest8i = [ 0x5a, 0xc6, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x17, 0x00, 0x00, 0x00 ];  // 0x0b,
		out.set(rest8i, k);
		k += rest8i.length;
		...
		k += 4;
		this.writeinttoout(out, (dateok ? 486: 442) + f.size + veroff, k);
		k += 4;
		...
		const rest10b = [ 0x0, 0x49, 0x6d, 0x42, 0x61, 0x63, 0x6b, 0x00, 0x00, 0x95, 0x09, 0x00, 0x00, 0xe2, 0x04,
			0x00, 0x00, 0x3b, 0xfb, 0xff, 0xff, 0xd0, 0x07, 0x00, 0x00, 0x21, 0xea, 0xff, 0xff, 0x13, 0x40,
			0x00, 0x00, 0x4f, 0xe9, 0xff, 0xff, 0x2f, 0x17, 0x00, 0x00, 0xcb, 0x1e, 0x00, 0x00, 0x12, 0x10,
			0x00, 0x00, 0x9d, 0x02, 0x00, 0x00, 0x20, 0x4e, 0x00, 0x00, 0x01, 0x0d, 0x00, 0x00, 0x4e, 0xc5,
			0x01, 0x00, 0x91, 0xfe, 0xff, 0xff, 0xc4, 0x09, 0x00, 0x00, 0x00, 0x26, 0x00, 0x00, 0x2d, 0x1c,
			0x00, 0x00 ];
		out.set(rest10b, k);
		k += rest10b.length;
