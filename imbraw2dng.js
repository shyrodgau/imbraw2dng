...
#version = "V3.5.0_30dbab0cae"; // actually const
...
addEntryRelative(tag, reltag, offset) {
	...
	if (null === o) {
		console.log('Rel: Old entry with tag ' + tag + ' not found!');
		return;
	}
	let t = TIFFOut.types.filter(v =>  v.t === o.xtype )[0];
	if (t.l !== 1 && offset !== 0) {
		console.log('Rel not possible because offset ' + offset + ' and length not 1 but ' + t.l);
		return;
	}
	if (o.ptr === undefined) {
		console.log('Rel not possible because reltag has no pointer');
		return;
	}
	if (t.l === 1 && (o.xcount - offset) <= 4) {
		console.log('Rel not possible because rest length < 4 ' + (o.xcount - offset));
		return;
	}
	let e = {
		xtag: tag,
		...
		xtype: o.xtype,
		type: [ 0, 0 ],
		xcount: (o.xcount - offset),
		count: [ 0, 0, 0, 0 ],
		ptr: o.ptr + (offset * t.l)
	};
	TIFFOut.writeshorttoout(e.tag, tag, 0);
	TIFFOut.writeshorttoout(e.type, o.xtype, 0);
